...
async function cargarCategoriasEnSelects() {
  try {
    ...
    const selects = document.querySelectorAll(
      'select[name="categoria_id"], select[name="categoria"], #categoria_id, #categoria, #sol-categoria, #prof-categoria, .select-categoria'
    );
...
function escucharFormulariosSolicitud() {
  document.addEventListener('submit', async (e) => {
    ...
    const esSolicitud =
      ...
      form.classList.contains('form-solicitud');
  ...
  try {
    ...
    const body = {
      ...
      zona:
        ...
        document.getElementById('sol-zona')?.value ||
        '',
      ...
      presupuesto:
        ...
        document.getElementById('sol-presupuesto')?.value ||
        null,

      fecha_preferida:
        formData.get('fecha_preferida') ||
        formData.get('fecha') ||
        document.getElementById('fecha_preferida')?.value ||
        document.getElementById('fecha')?.value ||
        document.getElementById('sol-fecha')?.value ||
        null,

      urgencia:
        formData.get('urgencia') ||
        document.getElementById('urgencia')?.value ||
        document.getElementById('sol-urgencia')?.value ||
        'Normal'
    };

    if (!body.titulo) {
      alert('Ingrese qué necesita.');
      return;
    }
