...
exports.obtenerMiPerfil = async (req, res) => {
  try {
    ...
    let perfilTrabajador = { rows: [] };

    if (await tablaExiste('perfiles_trabajador')) {
      perfilTrabajador = await pgPool.query(
        `
...
exports.misVerificaciones = async (req, res) => {
  try {
    ...
    if (!(await tablaExiste('verificaciones'))) {
      return res.json({
        ok: true,
        verificaciones: [],
        data: []
...
exports.subirVerificacion = async (req, res) => {
  try {
    ...
    if (!(await tablaExiste('verificaciones'))) {
      return res.status(500).json({
        ok: false,
        mensaje: 'La tabla verificaciones no existe en la base de datos.'
      });
    }
...
exports.obtenerPerfilPublico = async (req, res) => {
  try {
    ...
    if (await tablaExiste('calificaciones')) {
